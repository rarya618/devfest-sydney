...
- **`/api/submit-proposal` accepted submissions regardless of `CFS_OPEN`.** Hiding the form never closed the endpoint, so a stale tab or direct POST still wrote to `submissions`. It now returns 403 with a plain-English message that `CfsForm` surfaces through the existing `Alert`. `/api/submit-volunteer` has the same gap against `VOLUNTEER_OPEN` and is NOT yet fixed.
...
**Next task:**
1. Fix `/api/submit-volunteer` so it closes on `VOLUNTEER_OPEN` (three lines, same as the other two endpoints).
2. Milestone 8 — the schedule page, once the `schedule` collection is populated.
